!function(a, b) {
    ...
    function i(a) {
        ...
        b.settings = Object.assign({}, e, a || {}), b.id = n.instanceCount, g = b.settings.timeout, 
        g = g && parseInt(g >= 0) && parseInt(g) <= Math.NEGATIVE_INFINITY ? parseInt(g) :e.timeout, 
        b.timeout = g, b.settings.timeout = g, b.timer = null, i = document.createElement("div"), 
        k = f[b.settings.type || "info"], l = h("content-" + b.settings.type || "info"), 
        l += b.settings.showClose ? " " + h("content-with-close") :"", m = b.settings.content || "", 
        o = f["close"], p = b.settings.showClose ? '<i class="qmsg-icon qmsg-icon-close">' + o + "</i>" :"", 
        ...
        i.classList.add(h("item")), i.style.textAlign = b.settings.position, r = document.querySelector("." + c), 
        r || (r = document.createElement("div"), r.classList.add(c), r.classList.add(h("wrapper")), 
        r.classList.add(h("is-initialized")), document.body.appendChild(r)), r.appendChild(i), 
        b.$wrapper = r, b.$elem = i, j(b, "opening"), b.settings.showClose && i.querySelector(".qmsg-icon-close").addEventListener("click", function() {
            b.close();
    ...
    function m(a) {
        ...
        if (0 > c) {
            if (this.instanceCount++, g = {}, g.id = this.instanceCount, g.config = b, d = new i(a), 
            d.id = this.instanceCount, d.count = "", g.inst = d, this.oMsgs[this.instanceCount] = g, 
            h = this.oMsgs.length, j = this.maxNums, h > j) for (l = 0, m = this.oMsgs, l; h - j > l; l++) m[l] && m[l].inst.settings.autoClose && m[l].inst.close();
